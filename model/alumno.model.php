...
class AlumnoModel 
{
 ...
 private $bd;
    ...
    {
        $this->bd = new Conexion();
        $stmt = $this->bd->prepare("SELECT * FROM alumno where eliminado=0 and activo=1 order by primer_nombre asc;" );
        $stmt->execute();
    ...
    {
        $this->bd = new Conexion();
        $stmt = $this->bd->prepare("SELECT count(idAlumno)+1 as nro_persona FROM  alumno WHERE fecha_ingreso=:fecha_ingreso;");

...
class CursoModel 
{
 ...
 private $bd;
    ...
    {
        $this->bd = new Conexion();
        $stmt = $this->bd->prepare("SELECT * FROM curso where eliminado=0 and activo=1 order by nombre asc;" );
        $stmt->execute();
    ...
    {
        $this->bd = new Conexion();
        $stmt = $this->bd->prepare("SELECT count(idCurso)+1 as nro_persona FROM  curso WHERE fecha_ingreso=:fecha_ingreso;");
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_OBJ);      
